...
const addCurrencyOption = (currencyCode, container) => {
  const currencyInfo = exchangeRates[currencyCode];
  const { code, name } = currencyInfo;
...
const init = async () => {
  try {
    ...
    if (res.ok) {
      for (const currencyCode in data) {
        exchangeRates[currencyCode] = data[currencyCode].rate;
        addCurrencyOption(currencyCode, fromCurrency);
        addCurrencyOption(currencyCode, toCurrency);
      }
